...
export function TipTapFloatingMenu({ editor }: { editor: Editor }) {
  ...
  useEffect(() => {
    ...
    const mount = (el as { mount?: HTMLElement }).mount || el
    if (mount && 'addEventListener' in mount) {
      mount.addEventListener('keydown', handleEditorKeyDown)
    }
    return () => {
      const el = editor.options.element
      const mount = (el as { mount?: HTMLElement }).mount || el
      if (mount && 'removeEventListener' in mount) {
        ;(mount as HTMLElement).removeEventListener(
          'keydown',
          handleEditorKeyDown
        )
      }
