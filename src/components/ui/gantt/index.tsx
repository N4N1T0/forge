...
export const GanttHeader: FC<GanttHeaderProps> = ({ className }) => {
  ...
  return (
    <div
      className={cn(
        '-space-x-px flex h-full w-max divide-x divide-muted',
        className
      )}
    >
      <Header />
...
export const GanttFeatureItem: FC<GanttFeatureItemProps> = ({
  ...
  const [_previousMouseX, setPreviousMouseX] = useState(0)
  const [_previousStartAt, setPreviousStartAt] = useState(startAt)
  const [_previousEndAt, setPreviousEndAt] = useState(endAt)
  ...
  const handleItemDragMove = useCallback(() => {
    // Restrict moving the start date: no-op for main drag
    return
  }, [])
  ...
  return (
    ...
    >
      ...
      >
        {/* Left drag helper disabled to prevent start date changes */}
        {false && onMove && (
          <DndContext
